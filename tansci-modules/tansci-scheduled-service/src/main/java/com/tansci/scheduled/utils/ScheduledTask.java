...
import com.tansci.scheduled.service.TaskContextService;
 ...
 * @ClassPath： com.tansci.scheduled.task.ScheduledTask.java
 * @Description： 定时任务
 * @Author： tanyp
 * @Date： 2022/6/9 17:10
 **/
...
public class ScheduledTask implements SchedulingConfigurer {
    ...
    public void refreshTask(List<TaskConfig> tasks) {
        ...
        scheduledFutures.keySet().forEach(key -> {
            ...
            tasks.forEach(task -> {
                if (!Objects.equals(key, task.getTaskCode())) {
                    scheduledFutures.get(key).cancel(false);
                    scheduledFutures.remove(key);
                    cronTasks.remove(key);
                    return;
        ...
        tasks.forEach(item -> {
            ...
            // 任务已存在并且表达式未发生变化则跳过
            if (scheduledFutures.containsKey(item.getTaskCode()) && cronTasks.get(item.getTaskCode()).getExpression().equals(expression)) {
                return;
            }

            // 任务执行时间发生了变化，则删除该任务
            if (scheduledFutures.containsKey(item.getTaskCode())) {
                scheduledFutures.get(item.getTaskCode()).cancel(false);
                scheduledFutures.remove(item.getTaskCode());
                cronTasks.remove(item.getTaskCode());
            }
            ...
            CronTask task = new CronTask(new Runnable() {
                ...
                public void run() {
                    // 执行业务逻辑
                    try {
                        log.info("====执行单个任务，任务ID【{}】执行规则【{}】=======", item.getTaskCode(), item.getExpression());
                        taskContextService.execute(item.getServerName());
                    } catch (Exception e) {
            ...
            cronTasks.put(item.getTaskCode(), task);
            scheduledFutures.put(item.getTaskCode(), future);
        });
    }
